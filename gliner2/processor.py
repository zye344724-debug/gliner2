...
class SamplingConfig:
    ...
    NOTE: defaults tuned for schema-driven NER where field names are weak
    signals. For domain tasks like bond-deal extraction the field names are
    themselves the strongest semantic signal, so the JSON-structure sampling
    (synthetic labels / field removal / structure dropout / field shuffle) is
    disabled by default — it would otherwise make the training-time schema
    diverge from the inference-time schema.
    """
    # JSON Structures
    remove_json_structure_prob: float = 0.0
    shuffle_json_fields: bool = False
    remove_json_field_prob: float = 0.0
    # Entities
    remove_entities_prob: float = 0.0
    shuffle_entities: bool = False
    remove_entity_prob: float = 0.0
    synthetic_entity_label_prob: float = 0.0
    # Relations
...
class SchemaTransformer:
    ...
    def _process_json_structures(self, schema, schemas, labels, types, sampling):
        ...
        for parent, occurrences in groups.items():
            ...
            seen = set()
            for s in spans:
                key = tuple(self._hashable_value(x) for x in s)
                if key not in seen:
                    uniq.append(s)
                    seen.add(key)
    ...
    def _build_outputs(
            ...
            text_tokens: List[str],
            len_prefix: int,
            start_idx_map: Optional[List[int]] = None,
            end_idx_map: Optional[List[int]] = None,
    ) -> List[Dict]:
        ...
        for schema_tokens, task_type, struct_label in zip(
            ...
            if task_type != "classifications":
                ...
                for span in spans:
                    positions = []
                    for element in span:
                        positions.append(self._resolve_span_positions(
                            element, text_tokens, len_prefix,
                            start_idx_map, end_idx_map,
                        ))
    ...
    def _resolve_span_positions(
            ...
            text_tokens: List[str],
            len_prefix: int,
            start_idx_map: Optional[List[int]],
            end_idx_map: Optional[List[int]],
    ) -> List[Tuple[int, int]]:
        """Resolve a field value to word-span positions.

        If ``element`` is a span dict carrying explicit char offsets (the bond
        dataset), map those offsets to word indices directly — this disambiguates
        repeated surface strings (e.g. multiple '1000' / '+0' across deals in one
        sentence). Otherwise fall back to surface sublist matching.
        """
        if (
            isinstance(element, dict)
            and element.get("start") is not None
            and element.get("end") is not None
            and start_idx_map is not None
            and end_idx_map is not None
        ):
            return self._char_span_to_word_inclusive(
                element["start"], element["end"], start_idx_map, end_idx_map
            )

        if isinstance(element, list):
            nested = []
            for sub in element:
                nested.extend(self._resolve_span_positions(
                    sub, text_tokens, len_prefix, start_idx_map, end_idx_map
                ))
            return nested

        if str(element).startswith("[selection]"):
            return self._find_sublist(
                [str(element)[11:]], text_tokens[:len_prefix], case_insensitive=True
            )
        return self._find_sublist(self._tokenize_text(str(element)), text_tokens)

    def _char_span_to_word_inclusive(
            self,
            char_start: int,
            char_end: int,
        ...
        """Map a half-open char span to an inclusive word-index span.

        Mirrors ``char_span_to_word_boundaries`` semantics but returns the
        inclusive end used by the span loss (``width = end - start``), matching
        ``_find_sublist`` output.
        """
        n = len(start_idx_map)
        if n == 0:
            return [(-1, -1)]
        ...
        token_start = None
        for i in range(n):
            if start_idx_map[i] <= char_start < end_idx_map[i] \
                    or start_idx_map[i] == char_start:
                token_start = i
                break
            if start_idx_map[i] > char_start:
                token_start = i
                break
        ...
        for i in range(n - 1, -1, -1):
            if start_idx_map[i] < char_end <= end_idx_map[i] \
                    or end_idx_map[i] == char_end:
                token_end_incl = i
                break
            if end_idx_map[i] < char_end:
                token_end_incl = i
                break
        if token_end_incl is None or token_end_incl < token_start:
            return [(-1, -1)]
        return [(token_start, token_end_incl)]

    @staticmethod
    def _hashable_value(x: Any) -> Any:
        if isinstance(x, dict):
            return ("__span__", x.get("text"), x.get("start"), x.get("end"))
        if isinstance(x, list):
            return tuple(SchemaTransformer._hashable_value(i) for i in x)
        return x
